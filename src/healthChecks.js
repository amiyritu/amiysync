...
export async function checkShopifyHealth() {
  try {
    ...
    const response = await axios.head(`https://${domain}/admin/api/2024-10/shop.json`, {
      headers: {
...
export async function checkShiprocketHealth() {
  try {
    ...
    const response = await shiprocketGet('/v1/external/settlements', { limit: 1 });
...
export async function runAllHealthChecks() {
  ...
  const [shopify, shiprocket, sheets] = await Promise.all([
    ...
    checkShiprocketHealth().catch((e) => ({ status: false, message: e.message })),
    checkGoogleSheetsHealth().catch((e) => ({ status: false, message: e.message })),
  ]);
